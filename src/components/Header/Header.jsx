...
// import menuCLoseImg from "../../assets/Header/close-icon.png";
import menuCLoseImg from "../../assets/Header/close-icon.png";
...
function Header({ setShow, handleHeader }) {
  ...
  const [iconName, setIconName] = useState(menuCLoseImg);
  // const [iconName, setIconName] = useState(menuCLoseImg);

  const toggleMenu = () => {
    setMenuOpen(!menuOpen);
    
    // Cambia el nombre de la imagen del ícono según el estado del menú
    if (menuOpen) {
      setIconName(menuBlancoImg); // Cambia a la hamburguesa
    } else {
      setIconName(menuCLoseImg); // Cambia a la "X"
    }
  };
  ...
  return (
    <>
      <div className="hamburger-icon" onClick={handleHeader}>
        <img src={iconName} alt="Menu Icon" className="hamburger-icon" />
      </div>
